...
public class Solution {
    ...
    static void addUser(User user) {
        ...
        } else if (nameS != 0)System.out.println(UNKNOWN_ERROR);

        int ageS = user.setAge(age);
        if (ageS == -1) {
            System.out.println(CANNOT_BE_NEGATIVE);
        } else if (ageS == -2) {
            System.out.println(CANNOT_BE_TOO_BIG);
        } else if (ageS != 0) System.out.println(UNKNOWN_ERROR);
    ...
    static void findUserIndex(User user) {
        ...
        if (idx == -1) System.out.printf(NOT_FOUND, user.getName());
